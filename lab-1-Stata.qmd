...
format: 
  html:
    other-links:
      - text: "Solution .do file"
        href: "material/lab-1/ec3301-lab-1.do"
        icon: code-square
jupyter: nbstata
---
...
Download the "shs2023.dta" file. For this lab you do not need the "shs2023.csv" file.
:::
...
Find the downloaded file "shs2023.dta" in the "Downloads" folder. This is a Stata dataset saved in its custom .dta file format. Move the file from the Downloads folder to a location on either your OneDrive or 'Home Drive (H:)'. A good idea would be to create an EC3301 folder in the Documents folder of either drives. Then create a sub-folder for this lab: "..\Documents\EC3301\Lab 1\". Move `shs2023.dta` to this folder.
...
**Copy and paste the code from exercises 2-5 above** (i.e., the `rename` and `label` commands).
...
1. What is the average mortgage payment of households who own their home and provide a mortgage payment? [According to `mortgage_sum` there are three groups of households for whom we observe a mortgage payment. For the purposes of this question, you can ignore the third category: 3. "Buying with mortgage - amount given by respondent, but not used in imputation routines"] 
...
*| echo: true
cap log close
log using "[INSERT FILE PATH]\EC3301\Lab 1\lab-1.txt", replace text
```
...
*| eval: false
*| echo: true
graph export "file-name.png", replace
```
...
* Start log
cap log close
log using lab-1-log.txt, replace text

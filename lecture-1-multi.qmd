...
format: 
  html: 
    output-file: lecture-1.html
    other-links:
      - text: "PDF with Stata"
        href: "lecture-1-notes-Stata.pdf"
        icon: file-pdf
      - text: "PDF with R"
        href: "lecture-1-notes-R.pdf"
        icon: file-pdf
  revealjs: 
...
When we have a sample of observations, $i=1,\dots,n$, we typically use the notation:

$$
  y_i = \beta_0 + \beta_1x_{i1} + \beta_2x_{i2}+\dots + \beta_kx_{ik} + u_i
$$

We need to make further assumptions about sampling,
...
>
> The error term has expected value of 0 given the any vlaue of the explantory variable
>  
> $$
>    E[u | \mathbf{x}] = 0
> $$
...
::: {.fragment .fade-in}  
If sample is iid, then MLR-4 $\Rightarrow E[u_i | \mathbf{x}_i]$ for $i=1,\dots,n$. 
...
$$
  E[u | \mathbf{x}] = 0 \Rightarrow E[u_ix_{j}]=Cov(u, x_{j}) = 0\quad j=1,2,\dots,k
$$

##

3. MLR-4 implies that we can treat $\mathbf{x}$ as 'fixed' across repeated samples. 

   - If $x$ is fixed, there is only source of uncertainty is $u$. 
   - Allows us to treat $x$ as non-random in derivations; e.g. variance.
   
## MLR \#5

> **MLR-5: Homoskedasticity** 
>
> The variance of the error term is the same given any value of $x_i$. 
>  
> $$
...
::: {.fragment .fade-in}
If sample is iid, then MLR-4 $\Rightarrow Var(u_i|\mathbf{x}_i) = \sigma^2$ for $i=1,\dots,n$. 

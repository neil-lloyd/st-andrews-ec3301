...
* Plot
twoway (scatter hhcon hhinc, mc(black) mfc(%50)) (scatter fitted hhinc, mc(red) mfc(%50) msize(small) ms(S)) (scatter residual hhinc, mc(blue) mfc(%50) msize(small) ms(D))  (function y = _b[_cons] + _b[hhinc]*x, lc(red) lp(solid) range(0 3000)), ytitle("Household consumption (£/week)") xtitle("Household income (£/week)") title("Household consumption vs income (LCF, 2023)") legend(order(1 "Observed" 2 "Fitted values" 3 "Residuals") r(3) pos(2) ring(0)) yline(0, lc(blue) lp(solid))
```
...
#| echo: false
#| output: false
graph export "material/lecture-2/lcf-decomp.png", replace
```
![](material/lecture-2/lcf-decomp.png){fig-align="center" width=67%}
...
ggplot(plot_vals, aes(x = hhinc, y = hhcon)) +
  ...
  geom_hline(yintercept = 0, colour = "blue", linewidth = 0.7) +
...
$$
  \bar{\mathbf{R}}^2 = 1 - \frac{\text{SSR}/(n-k-1)}{\text{SST}/n}
$$
...
::: {.fragment .fade-in}
As you increase $k$, you decrease $\bar{\mathbf{R}}^2$

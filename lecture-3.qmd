...
$$
  \hat{\beta}_j|\mathbf{x} \sim\quad \mathcal{N}\big(\beta_j,\,Var(\hat{\beta}_j|\mathbf{x})\big) 
$$

Why?

- $u_i|\mathbf{x} \sim \mathcal{N}(0,\sigma^2)$
...
$$
  \frac{\hat{\beta}_j-\beta_j}{se(\hat{\beta}_j)} | x \sim \mathcal{N}(0,1)
$$
...
This term - let's call it $z=(\hat{\beta}_j-\beta_j)/se(\hat{\beta}_j)$ is another random variable. Since we know the properties $\mathcal{N}(0,1)$, we know:

::: incremental
- $Pr(z<\text{z}_{0.025}) = 0.025$ where $\text{z}_{0.025} = -1.96$ is the $2.5^{th}$ percentile of $\mathcal{N}(0,1)$

- $Pr(z>\text{z}_{0.975}) = 0.025$ where $\text{z}_{0.975} = 1.96$ is the $97.5^{th}$ percentile of $\mathcal{N}(0,1)$
...
$$
  \text{Test-stat} = \frac{\hat{\beta}_j-0}{se(\hat{\beta}_j)} | x \underset{H_0}{\sim} \mathcal{N}(0,1)
$$
...
$$
  \text{Test-stat} \underset{H_0}{\sim} \mathcal{N}(0,1)
$$
...
$$
  \begin{aligned}
    ...
    =& Pr\Bigg(\underbrace{\frac{\hat{\beta}_j-\kappa}{se(\hat{\beta}_j)}}_{\sim \mathcal{N}(0,1)}<-1.96-\frac{\kappa}{se(\hat{\beta}_j)}\Bigg|\beta_j = \kappa\Bigg) + Pr\Bigg(\underbrace{\frac{\hat{\beta}_j-\kappa}{se(\hat{\beta}_j)}}_{\sim \mathcal{N}(0,1)}>1.96-\frac{\kappa}{se(\hat{\beta}_j)}\Bigg|\beta_j = \kappa\Bigg) \\

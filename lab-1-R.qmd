...
format: 
  html: 
    other-links:
      - text: "Solution .R file"
        href: "material/lab-1/ec3301-lab-1.R"
        icon: code-square  
engine: knitr  
---
...
:::{.callout-important title="Data Access"}
For this lab you will be using a dataset supplied by the UK Data Service under an End User License. Before accessing the dataset, you need to do ONE of the following:

- go to [UK Data Service](https://beta.ukdataservice.ac.uk/myaccount/login) and register an academic account using "University of St Andrews" as your institution.

- OR sign the physical license agreement available to you in class. 

Once you have have completed either of these, you can access the data via Moodle so long as you: 

- agree to the [Data Access Quiz](https://moody.st-andrews.ac.uk/moodle/mod/quiz/view.php?id=1563763).

Upon submitting the quiz, you will be shown a link to the data.

Download both the "shs2023.dta" and "shs2023.csv" files. 
:::
...
R is an open source programming language developed for statistical analysis. Estimates of R usage in Economics suggest that only 9.8% of replication packages in economic journals contain .R scripts (i.e. R code) (Source: [*Economics and R Blog*](https://skranz.github.io/r/2023/12/29/FindingEconomicArticles7.html)). Stata remains dominant (72%), with Matlab second (24%).

**Why not R? It's free!?** Part of the story is network 'lock-in': academics train students to use the tools they know, which is Stata and Matlab. But R (and Python) usage is rising; in particular, among younger researchers. And this is happening for a few key reasons:
...
An advantage of using R is that it is a free open-source language. You will have remote access to R and the RStudio IDE via [AppsAnywhere](https://appstore.st-andrews.ac.uk/) on the desktop computers in this lab. Unfortunately, AppsAnywhere will not allow you install additional user-written packages, but does have a range of popular packages pre-installed. This lab is designed to run on this system. 

Of course, there is nothing stopping you installing R on your own computer. You will first need to [install R](https://cran.r-project.org/) and then an IDE. R comes with its own, fairly basic, IDE but I recommend using [RStudio](https://posit.co/products/open-source/rstudio). If you are a more experienced programmer, you can also consider [Positron](https://posit.co/products/ide/positron).[^1]
...
**Why not Python?** R is used by the Statistics and Maths modules that count as optional prerequisites for this module. But more importantly, R has a well developed stock of econometrics packages through CRAN. Python is more widely used in Machine Learning and has excellent data visualization packages. If needs be, you can always run Python commands within R via the `reticulate` package. 
...
Find the downloaded "shs2023.dta" and "shs2023.csv" files in the "Downloads" folder. The `.dta` file is a Stata dataset that contains a extra metadata information. The latter is a simpler plain text copy of the file, without this metadata. Move the files to your project folder. 
...
#| output: false
#| eval: true
shs_csv <- read.csv(file.path(data_dir, "shs2023.csv"))
```
...
#| eval: true
library(haven)
shs_dta <- read_dta(file.path(data_dir, "shs2023.dta"))
```
...
The labels attached these variables are equally uninformative. 

4. Label `dwell_type` "Type of dwelling"

The base R option is:
...
Alternatively, you can use the `labelled()` function within the `haven` package.
...
:::{.callout-note}
The package `labelled` - developed by the same team as `haven` - can do this more efficiently. However, you will not be able to do this in AppsAnywhere as it would require installation. 

```{r}
#| eval: false
install.pacakges("labelled")
library("labelled")

var_label(clean) <- list(
...
You can see how this would reduce the number of lines need to perform a sequence of dataset manipulations. We can even incorporate the label changes using the `mutate()` function:
...
You will notice that the labels of the values attached to `MD20QUIN` are incomplete. 

```{r}
clean %>% count(MD20QUIN)
```

If you are able to load the `labelled` package, you can also list the value labels using `val_labels(clean$MD20QUIN)`. We can create our own label and attach them to this variable using the `labelled()` function from `haven`:
...
:::{.callout-note}
Again, the `labelled` package has a simpler notation for this: 
